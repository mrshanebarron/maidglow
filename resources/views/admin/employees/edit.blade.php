            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.employees.update', $employee) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="role" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-[#5f7360] focus:border-purple-500">
                                <option value="tech" {{ old('role', $employee->role) === 'tech' ? 'selected' : '' }}>Technician</option>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    @foreach($colors as $color)
                                        <label class="cursor-pointer">
                                            <input type="radio" name="color" value="{{ $color }}" class="sr-only peer" {{ old('color', $employee->color) === $color ? 'checked' : '' }}>
                    ...
                    <div class="flex justify-end gap-4 pt-4 border-t">
                        <a href="{{ route('admin.employees.show', $employee) }}" class="px-4 py-2 text-gray-600 hover:underline">Cancel</a>
